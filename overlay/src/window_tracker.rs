...
impl WindowTracker {
    ...
    pub fn update(&mut self) -> bool {
        ...
        let target_is_foreground =
            foreground_window == self.target_hwnd || foreground_window == self.overlay_hwnd;
        if target_is_foreground != self.currently_visible {
            self.currently_visible = target_is_foreground;
            unsafe {
                SetLayeredWindowAttributes(
                    self.overlay_hwnd,
                    COLORREF(0),
                    if target_is_foreground { 255 } else { 0 },
                    LWA_ALPHA,
                );
            }
        }
